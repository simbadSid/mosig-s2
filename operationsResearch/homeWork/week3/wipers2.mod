...
//        hasWorked[m] == (timeOfWork[m] > 0);
  ...
  // Constraint on using A or C but not both
  (hasWorked[1] + hasWorked[3]) <= 2;

  // Constraint on using B and C
//  timeOfWork[3] >= hasWoked[2] * cThresholdIfB;
}
